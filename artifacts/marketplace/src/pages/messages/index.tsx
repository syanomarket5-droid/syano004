...
import { MessageCircle, Send, ArrowLeft, ArrowRight, Store } from "lucide-react";
    ...
    <div className={`flex ${isMine ? "justify-end" : "justify-start"} mb-2`}>
      ...
      >
        ...
        <p className={`text-[10px] mt-1 ${isMine ? "text-primary-foreground/60" : "text-muted-foreground"}`}>
          ...
          {isMine && msg.readAt && t("messages.read_receipt")}
        </p>
      </div>
    </div>
  );
      ...
      <div className="p-4 border-b flex items-center gap-3 bg-card shrink-0">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {t("messages.seller_label")}
            {conv.productId ? t("messages.about_product") : ""}
          </p>
        </div>
      </div>
...
export default function MessagesPage() {
  ...
  /*
   * WhatsApp-style full-viewport chat layout:
   *
   *  ┌─────────────────────────────────────────┐  ← 100dvh outer shell
   *  │ <Navbar />                  shrink-0    │  ← ~60px mobile / 64px desktop
   *  ├─────────────────────────────────────────┤
   *  │ <main>  flex-1 overflow-hidden          │  ← fills remaining dvh exactly
   *  │  ┌───────────────────────────────────┐  │
   *  │  │ title row            shrink-0     │  │
   *  │  ├───────────────────────────────────┤  │
   *  │  │ chat panel  flex-1 min-h-0        │  │  ← fills remaining, no px math
   *  │  │  sidebar │ thread                 │  │
   *  │  │          │  header    shrink-0    │  │
    ...
    >
      ...
      <main className="flex-1 overflow-hidden flex flex-col min-w-0 min-h-0">
        <div className="flex-1 overflow-hidden flex flex-col p-4 min-h-0 max-w-6xl mx-auto w-full">
          ...
          {/* Page title — shrink-0 so it never collapses */}
          <div className="flex items-center gap-3 mb-4 shrink-0">
            <MessageCircle className="h-5 w-5 text-primary" />
            <h1 className="text-xl font-bold">{t("messages.title")}</h1>
          </div>

          {/* Chat panel — flex-1 min-h-0 lets it fill the remaining space exactly */}
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1 min-h-0">
                  {isLoading ? (
                    ...
                    <div className="text-center py-12 text-muted-foreground px-4">
                      ...
                      <p className="text-xs mt-1">{t("messages.no_conversations_hint")}</p>
                      <Link href="/products">
              ...
              <div className={`flex-1 min-w-0 min-h-0 ${activeConv ? "flex" : "hidden sm:flex"} flex-col overflow-hidden`}>
                {activeConv ? (
                  <>
                    ...
                    <MessageThread conv={activeConv} userId={user.id} />
                  </>
                ) : (
